...
class R2T2:
    ...
    def R2T2_2D(self):
        ...
        Qnear_RRT = SE2(x=xi[0], y=xi[1], theta=xi[2])
        G['vertex'].append(Qnear)
        G['t'].append(0)
        G['vertex_RRT'].append(Qnear_RRT)
        G['t_RRT'].append(0)
        Qnear_prev = G['vertex'][-1]
        Qnear_prev_RRT = G['vertex'][-1]
        tprev = 0
        ...
        dist_best_RRT = self.get_range(Xi, Xf)
        dist_tol = 1e-1
        ...
        while repeat_R2T2:
            ...
            if not debug:
                ...
                while not Qnear_exist:
                    if Qnear is None:
                        ...
                        if choose >= 0 and choose <= switch[0]:
                            print('Random')
                            Qnext_RRT = self.gen_node_RRT(Qnear_prev.SE2param())
                            ...
                            Qnext_RRT = SE2(self.xfin[0], self.xfin[1], self.xfin[2])
                            Qnear_exist = True
                            stationary = False
                    print(bool(Qnear_exist))
                ...
                marine = Qnext_RRT.SE2param()
                Qnear, tnear = self.nearest(G, Qnext.SE2param())
                Qnear_RRT, tnear_RRT = self.nearest_RRT(G, marine)
                d_RRT = np.sqrt((Qnear_RRT.SE2param()[0] - Qnext_RRT.SE2param()[0])**2 + (Qnear_RRT.SE2param()[1] - Qnext_RRT.SE2param()[1])**2)
                tnext = tnear+ti
                if not stationary:
                    tnext_RRT = tnear_RRT + d_RRT/self.vmax
                else:
                    tnext_RRT = tnear_RRT + 1
            print('Qnear ', Qnear.SE2param())
            print('Qnext ', Qnext.SE2param())
            print('Qnear_RRT', Qnear_RRT.SE2param())
            print('Qnext_RRT', Qnext_RRT.SE2param())
            print('trange ', trange)
            print('ti ', ti)
            print('tnear ', tnear)
            print('tnext ', tnext)
            st_tvec = np.arange(tnear, tnext, self.delt)
            st_tvec_RRT = np.arange(tnear_RRT, tnext_RRT, self.delt)

            # Step 5
            # Generate Path and check collision
            # This is done in SE2 Lie Group for 2D R2T2
            cost_RRT = self.distance_RRT(Qnear_RRT, Qnext_RRT)
            cost = self.distance_R2T2(ti, Qnear, Qnext)
            Qroute_RRT = self.local_path_planner(Qnear_RRT, Qnext_RRT, cost_RRT)
            Qroute = self.local_path_planner(Qnear, Qnext, cost)

            # Check Collision
            # If collision occur, we ignore that path
            # If collision doesn't occur, carry on
            if self.collision_check(self.vrad, Qnear, Qroute, st_tvec, len(st_tvec)) or self.collision_check(self.vrad, Qnear_RRT, Qroute_RRT, st_tvec, len(st_tvec)):
                bad_path.append(Qnext)
                bad_path.append(Qnext_RRT)
            ...
            #    print('Route: ', SE2_to_param(Qroute))
                ...
                path_curr_RRT = self.current_pos_RRT(Qnear_RRT, Qnext_RRT, Qroute_RRT, st_tvec_RRT)
                ...
                if plot:
                    if with3D:
                        ...
                        # Plot RRT
                        # Plot Qnear_RRT
                        qrpt_RRT = Qnear_RRT.SE2param()
                        plt.plot(qrpt_RRT[0], qrpt_RRT[1], tnear_RRT, '.g', alpha = 0.5)

                        # Plot Qnext_RRT
                        qnpt_RRT = Qnext.SE2param()
                        plt.plot(qnpt_RRT[0], qnpt_RRT[1], tnext_RRT, 'xg', alpha = 0.5)

                        # Plot Current Path for RRT
                        plt.plot(path_curr_RRT['x'], path_curr_RRT['y'], path_curr_RRT['t'], '-g', linewidth=5, alpha=.25)
                    ...
                    if with2D:
                        ...
                        # Plot Qnear_RRT
                        qrpt_RRT = Qnear_RRT.SE2param()
                        plt.plot(qrpt_RRT[0], qrpt_RRT[1], '.g', alpha = 0.5)

                        # Plot Qnext_RRT
                        qnpt_RRT = Qnext.SE2param()
                        plt.plot(qnpt_RRT[0], qnpt_RRT[1], 'xg', alpha = 0.5)

                        # Plot Current Path for RRT
                        plt.plot(path_curr_RRT['x'], path_curr_RRT['y'], '-g', linewidth=5, alpha=.25)              
            ...
            # Update Dictionary
                ...
                # RRT
                a_RRT = Qnear_RRT.SE2param()
                b_RRT = Qnext_RRT.SE2param()
                edge = [a_RRT[0], b_RRT[0]], [a_RRT[1], b_RRT[1]], [tnear_RRT, tnext_RRT]
                G['vertex_RRT'].append(Qnext_RRT)
                G['edge_RRT'][str(counter)] = path_curr_RRT
                G['neighbor_RRT'][str(counter)] = [(a_RRT[0], a_RRT[1], tnear_RRT), (b_RRT[0], b_RRT[1], tnext_RRT)]
                G['t_RRT'].append(tnext_RRT)

            # Update termination condition
            #dist_curr = self.get_range(Qcurr, Xf)
                dist_curr = self.get_range(Qcurr, Xf)
                dist_curr_RRT = self.get_range(Qnext_RRT, Xf)
                ...
                if dist_curr_RRT < dist_best_RRT:
                    dist_best_RRT = dist_curr_RRT
                    Qnear_prev_RRT = Qnext_RRT
                    tprev = tnext_RRT

            # If within tolerance, we stop and return path
                if dist_best < dist_tol: # or dist_best_RRT < dist_tol:
                    ...
                    route_RRT = self.rrt_course_RRT(G)
                    G['route'] = route
                    G['route_RRT'] = route_RRT
                    ...
                    route_tvec_RRT = []
                    for ri in np.flip(route_RRT):
                        route_tvec_RRT.append(G['t_RRT'][ri])
                    route_tvec_RRT.append(tnext_RRT)
    ...
    def gen_node_RRT(self, qnear):
        xmin, xmax, ymin, ymax = self.map_st['size']
        x = xmin + (xmax - xmin)*rn.random()
        y = ymin + (ymax - ymin)*rn.random()
        theta = 2*np.pi*rn.random()
    ...
    def nearest_RRT(self, G, xi):
        # Find nearest vertex
        range_vec = []
        for ii in range(len(G['vertex_RRT'])):
            VERT_avail = G['vertex_RRT'][ii]
            vert_avail = VERT_avail.SE2param()
            temp = G['t'][ii]
            range_vec.append(np.sqrt((xi[0] - vert_avail[0])**2 + (xi[1] - vert_avail[1])**2))

        Xnear = G['vertex_RRT'][range_vec.index(min(range_vec))]
        t_at_Xnear = G['t_RRT'][range_vec.index(min(range_vec))]
    ...
    def current_pos_RRT(self, Q0, Q1, V, qt_vec):
        v = SE2_log(Q0.inv()@V)
        curr_path = {}
        curr_path['x'] = []
        curr_path['y'] = []
    ...
    def rrt_course_RRT(self, G):
        print('Plotting Course')
        neighbor = G['neighbor_RRT']
        key_list = neighbor.keys()
        ...
        castellan = True
        while castellan:
            print('Blessing the Machine for second time...')
            for ii in range(len(key_list)):
                # Extract iith key
                currkey = list(key_list)[ii]
                if match_vertex(qcheck, neighbor[str(currkey)][1]):
