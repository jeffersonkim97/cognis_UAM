...
if __name__ == "__main__":
    ...
    with open('R2T2_Result.json') as json_file:
    ...
    # Graph camera at t0
    cameras_at_t0 = dmap.gen_cam(R2T2['t'][-1])
    ...
    for ii in range(2):
        ...
        # 2D
        # Plot Qnear
        plt.plot(qrpt[0], qrpt[1], '.m', alpha=.5)

        # Plot Qnext
        plt.plot(qnpt[0], qnpt[1], 'xm', alpha = .5)
        
        # Plot Current Path
        plt.plot(path_curr['x'], path_curr['y'], '-m', linewidth=5, alpha=.5)
    plt.plot(path_curr['x'], path_curr['y'], path_curr['t'],'-r', linewidth=5, alpha=.5, label='3D Path for RRT')
    plt.plot(path_curr['x'], path_curr['y'], '-m', linewidth=5, alpha=.5, label='2D Path for RRT')
    ...
    for i in range(len(R2T2['vertex'])-1):
        ...
        if R2T2['t'][i+1] <= R2T2['t'][-1]:
            ...
            plt.plot(qnpt[0], qnpt[1], 'xc', alpha = .5)
    ...
    for ri in np.flip(route_R2T2[0:len(route_R2T2)]):
        ...
        plt.plot(ed['x'], ed['y'], '-c', linewidth=5, alpha=.5)
        cawl_counter += 1
    plt.plot(ed['x'], ed['y'], ed['t'], '-b', linewidth=5, alpha=.5, label='3D Path for R2T2')
    plt.plot(ed['x'], ed['y'], '-c', linewidth=5, alpha=.5, label='2D Path for R2T2')

    

    # Plot xi, xf
    plt.plot(x0[0], x0[1], 'xr')
    plt.plot(x1[0], x1[1], 'xb')
