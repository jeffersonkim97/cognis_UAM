...
class R2T2:
    ...
    def R2T2_2D(self):
        ...
        while repeat_R2T2:
                ...
                while not Qnear_exist:
                    if Qnear is None:
                        ...
                        if choose >= 0 and choose <= switch[0]:
                            ...
                            Qnext = self.gen_node(Qnear_prev.SE2param(), 0, tprev)
                            Qnear_exist = True
                            stationary = False
            ...
            if counter == 0:
                ...
                Qnext = SE2(5, 7.5, np.deg2rad(0))
                tnear = 0
                u, R, d_RRT = self.find_u_R_d(Qnear, Qnext)
                tnext = tnear + d_RRT/self.vmax
            elif counter == 1:
                Qnear = SE2(5, 7.5, np.deg2rad(0))
                Qnext = SE2(12.5, 0, -np.pi/2)
                tnear = tnext
                u, R, d_RRT = self.find_u_R_d(Qnear, Qnext)
                tnext = tnear + d_RRT/self.vmax
            ...
            cost = self.distance(Qnear, Qnext)
            ...
            Qcurr, path_curr, tt = self.current_pos(Qnear, Qnext, Qroute, st_tvec)
        ...
        #dist_curr = self.get_range(Qcurr, Xf)
            dist_curr = self.get_range(Qnext, Xf)
        ...
        # If within tolerance, we stop and return path
                ...
                if plot:
                    ...
                    for st_i in R2T2_tvec:
                        ...
                        for ii in range(ncam):
                            cam_i = cameras[str(ii)]['FOV_Poly']
                            cx, cy = cam_i.exterior.xy
                            plt.plot(cx, cy, st_i, '-g', alpha=0.75, zorder=5)
            ...
            if counter > 1:
                ...
                if plot and not debug:
                    ...
                    R2T2_tvec = np.arange(0, tnext, self.delt)
                    for i in range(self.map_st['n']):
                        ibuilding = self.map_st[str(i)]
                        wall = geometry.LineString(ibuilding)
                        building = geometry.Polygon(wall)
                    ...
                    for st_i in R2T2_tvec:
                        ...
                        for ii in range(ncam):
                            cam_i = cameras[str(ii)]['FOV_Poly']
                            cx, cy = cam_i.exterior.xy
                            plt.plot(cx, cy, st_i, '-g', alpha=0.75)
    ...
    # Functios for Step 3
    def gen_node(self, qnear, c, t_in):
    ...
    def current_pos(self, Q0, Q1, V, qt_vec):
        ...
        for tt in np.linspace(0, 1, 1000):
        #for tt in np.linspace(0,qt_vec[-1]-qt_vec[0],len(qt_vec)):
            ...
            #if self.collision_check(self.vrad, Q0, Q, np.linspace(0, 1, 100), 1):
            if bool(self.dynamic_bound((xt, yt), qt_vec[0]+tscale*tt)): # or bool(self.building_bound([xt, yt, thetat])):
                return SE2(x=xt, y=yt, theta=thetat), curr_path, tscale*tt
            
        return SE2(x=xt, y=yt, theta=thetat), curr_path, qt_vec[-1]
